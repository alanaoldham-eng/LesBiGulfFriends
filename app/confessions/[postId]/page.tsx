...
export default function ConfessionDetailPage() {
  ...
  const [postSummary, setPostSummary] = useState<Record<string, number>>({
    heart: 0,
    hug: 0,
    same: 0,
    support: 0,
  ...
  const refresh = async (currentUserId: string) => {
    ...
    for (const reply of replyRows) {
      summaries[reply.id] = await getAnonymousReactionSummary({
        replyId: reply.id,
      }).catch(() => ({
        heart: 0,
        hug: 0,
  ...
  const sendReply = async () => {
    if (!me || !postId || !replyBody.trim()) return;
    ...
    <ClientShell>
      <section className="hero">
        ...
        <p style={{ fontSize: 16, lineHeight: 1.6, opacity: 0.9 }}>
          Posts and replies are anonymous to members.
        </p>
      </section>
      ...
      <div className="grid">
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 12 }}>
                {REACTIONS.map((reaction) => (
                  <button
                    ...
                    onClick={() => reactPost(reaction.key)}
                  >
                    {reaction.label} {postSummary[reaction.key] || ""}
                  </button>
                ))}
                <button className="button secondary" onClick={reportPost}>
                  Report
                </button>
              </div>
            </>
        ...
        >
          ...
          {!post?.is_locked ? (
            ...
            <p style={{ margin: 0, opacity: 0.8 }}>
              Replies are locked on this post.
            </p>
          )}
          ...
          {replies.length ? (
            replies.map((reply) => (
              <div
                key={reply.id}
                style={{ borderBottom: "1px solid #f1dfe8", padding: "12px 0" }}
              >
                <div style={{ fontSize: 12, opacity: 0.65 }}>Anonymous</div>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 10 }}>
                  {REACTIONS.map((reaction) => (
                    <button
                      key={reaction.key}
                      className="button secondary"
                      onClick={() => reactReply(reply.id, reaction.key)}
                    >
