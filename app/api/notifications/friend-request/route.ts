...
export async function POST(request: NextRequest) {
  try {
    const { recipientUserId, requesterName } = await request.json();
    ...
    if (!settings?.[0]?.email_friend_requests) {
    ...
    const html = `<div style="font-family: Arial, sans-serif; line-height: 1.6;"><h2>New friend request</h2><p>${requesterName || "A member"} sent you a friend request.</p><p><a href="${siteUrl}/friends" style="display:inline-block;padding:12px 16px;background:#8d2d5d;color:#fff;text-decoration:none;border-radius:10px;">Open Friends</a></p></div>`;
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: "You got a new friend request",
        html,
      }),
    });
