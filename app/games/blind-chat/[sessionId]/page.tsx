...
export default function BlindChatSessionPage() {
  ...
  const send = async () => {
    ...
    setBody("");
    setStatus("Message added locally. Wire realtime/database next.");
  };
    ...
    <ClientShell>
      ...
      <div className="grid">
        ...
        >
          ...
          <p style={{ opacity: 0.8 }}>
            Your alias in this chat is <strong>{myAlias}</strong>.
          </p>
          ...
          >
            {messages.length ? (
                ...
                >
                  <div
                    style={{
                      ...
                      background: m.sender === "me" ? "#fdebf3" : "#ffffff",
                    }}
                  >
                    <div style={{ fontSize: 12, opacity: 0.6, marginBottom: 4 }}>
                      {m.sender === "me" ? myAlias : partnerAlias}
                    </div>
              ...
              <p style={{ margin: 0, opacity: 0.7 }}>
                No messages yet. Start the conversation anonymously.
              </p>
            )}
          </div>
          ...
          <div style={{ display: "grid", gap: 12, marginTop: 12 }}>
            <textarea
              id="blind-chat-message"
              name="blindChatMessage"
              value={body}
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <button className="button" onClick={send} disabled={!me || !canSend}>
                Send
              </button>
              <button
                className="button secondary"
                onClick={() => setStatus("Reveal request UI shell installed.")}
                type="button"
              >
                Request Reveal
              </button>
              <button
                className="button secondary"
                onClick={() => setStatus("Leave flow UI shell installed.")}
                type="button"
              >
                Leave Chat
              </button>
