...
create index if not exists idx_event_media_moderation_status on public.event_media(moderation_status);
create index if not exists idx_group_messages_moderation_status on public.group_messages(moderation_status);
create index if not exists idx_event_messages_moderation_status on public.event_messages(moderation_status);
create index if not exists idx_waiting_room_moderation_status on public.waiting_room_candidates(moderation_status);
...
declare
  ...
  _actor_email text := coalesce(auth.jwt() ->> 'email', null);
begin
  ...
  if _target_type = 'event_media' then
    update public.event_media set moderation_status='removed', removed_reason=trim(_reason), removed_by=_actor, removed_by_email=_actor_email, removed_at=now() where id=_target_id;
  elsif _target_type = 'group_messages' then
    update public.group_messages set moderation_status='removed', removed_reason=trim(_reason), removed_by=_actor, removed_by_email=_actor_email, removed_at=now() where id=_target_id;
  elsif _target_type = 'event_messages' then
    update public.event_messages set moderation_status='removed', removed_reason=trim(_reason), removed_by=_actor, removed_by_email=_actor_email, removed_at=now() where id=_target_id;
  elsif _target_type = 'waiting_room_candidates' then
    update public.waiting_room_candidates set moderation_status='removed', status='removed', removed_reason=trim(_reason), removed_by=_actor, removed_by_email=_actor_email, removed_at=now() where id=_target_id;
  else
  ...
  insert into public.moderation_removal_log(target_type,target_id,removed_by,removed_by_email,reason)
  values(_target_type,_target_id,_actor,_actor_email,trim(_reason));
...
where id in ('event-media','event_media','event-gallery','event_gallery','chat-media','profile-photos');
