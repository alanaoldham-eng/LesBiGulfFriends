...
-- =========================================
-- EVENT MEDIA COMMENTS / REACTIONS
-- =========================================
...
create unique index if not exists event_media_reactions_unique_user_emoji
  on public.event_media_reactions(media_id, user_id, emoji);

create index if not exists idx_event_media_comments_media_created
  on public.event_media_comments(media_id, created_at desc);

create index if not exists idx_event_media_reactions_media
  on public.event_media_reactions(media_id);

-- =========================================
-- MESSAGE PERFORMANCE
-- =========================================

create index if not exists idx_messages_sender_recipient_created_desc
...
-- =========================================
-- RPC: OWNER CAN TOGGLE MODERATOR
-- =========================================

create or replace function public.set_group_moderator_status_rpc(
  _group_id uuid,
  _user_id uuid,
...
declare
  ...
  _actor_role text;
  _next_role text := case when _make_moderator then 'mod' else 'member' end;
  _new_role text;
begin
  ...
  where group_id = _group_id and user_id = _actor
  limit 1;

  if _actor_role is distinct from 'owner' then
    raise exception 'Only the owner can change moderator status';
  end if;
...
-- =========================================
-- RPC: OWNER/MOD CAN REMOVE MEMBER
-- =========================================

create or replace function public.remove_member_from_group_rpc(
  _group_id uuid,
  _user_id uuid,
  _reason text
) returns jsonb
...
begin
  ...
  if _actor_role not in ('owner', 'mod') then
    raise exception 'You are not allowed to remove members';
  end if;
  ...
  if _actor_role = 'mod' and _target_role <> 'member' then
    raise exception 'Moderators may only remove members';
  end if;
  ...
  if not found then
    raise exception 'Member was not removed from the group.';
  end if;

  update public.profiles
     set membership_status = 'removed',
         removed_reason = trim(_reason),
         removed_at = now(),
         removed_by = _actor
   where id = _user_id;
...
-- =========================================
-- RPC: DELETE EVENT OWNED BY CURRENT USER
-- =========================================

create or replace function public.delete_owned_event_rpc(
  _event_id uuid
) returns jsonb
language plpgsql
security definer
...
begin
  ...
  if _event.created_by <> _actor then
    raise exception 'Event not owned by current user';
  end if;

  delete from public.events where id = _event_id;

  if not found then
    raise exception 'Event was not deleted.';
  end if;
...
-- =========================================
-- RPC: DELETE WAITING ROOM CANDIDATE
-- =========================================

create or replace function public.delete_waiting_candidate_rpc(
  _candidate_id uuid
) returns jsonb
language plpgsql
security definer
...
as $$
begin
  delete from public.waiting_room_candidates
